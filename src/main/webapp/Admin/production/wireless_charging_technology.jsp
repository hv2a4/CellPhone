...
<body>
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								<div class="modal-header">
									<h1 class="modal-title fs-5" id="themLabel">Thêm công nghệ
										sạc mới</h1>
						...
						<div class="col-sm-12">
							<div class="card-box table-responsive">
								<table id="datatable-responsive"
									...
									<thead>
										<tr>
											<th>Mã công nghệ sạc</th>
											<th>Tên công nghệ sạc</th>
											<th class="text-center">Hoạt động</th>
										</tr>
									</thead>
									<tbody>
										<c:forEach var="item"
											items="${list_wireless_charging_technology}">
											<tr>
												<td>${item.ID }</td>
												<td>${item.NAME }</td>
												<td class="text-center"><i
													onclick="getwireless_charging_technologyById(${item.ID })"
									...
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-body">
												<form:form class="form-horizontal form-label-left"
													method="POST"
													modelAttribute="wireless_charging_technologyUpdate"
													...
													<div class="row text-left">
														...
														<div class="form-group">
															<label class="control-label">Tên công nghệ sạc</label>
															<form:input id="wireless_charging_technologyName"
	...
	<script>
		function getwireless_charging_technologyById(id) {
		    $.ajax({
		        type: "GET",
		        url: "/admin/ajax/getwireless_charging_technology/" + id,
		        success: function(wireless_charging_technology) {
		            // Đổ dữ liệu vào form:form
		            $("#wireless_charging_technologyId").val(wireless_charging_technology.id);
		            $("#wireless_charging_technologyName").val(wireless_charging_technology.name); 
		...
		function modelDelete(id) {
		    $.ajax({
		        ...
		        success: function(wireless_charging_technology) {
	                ...
	                $("#thongbao").text("Bạn có muốn xóa công nghệ sạc " + wireless_charging_technology.name);
	                var modal = new bootstrap.Modal(document.getElementById('delete'));
	                modal.show();
	                
	                // Xử lý khi nhấn nút Delete
	                $("#deleteButton").on("click", function() {
	                    window.location.href = "/admin/wireless_charging_technology/delete?id=" + wireless_charging_technology.id;
	                });
		        },
		        error: function(xhr, status, error) {
		            console.log("Error: " + error);
	...
	<c:if test="${message eq 'Hoàn tất'}">
		<script>
			...
			setTimeout(function() {
				window.location.href = "/admin/wireless_charging_technology";
			}, 1800);
