...
<body>
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								<div class="modal-header">
									<h1 class="modal-title fs-5" id="themLabel">Thêm phương thức thanh toán mới</h1>
									<button type="button" class="btn-close" data-bs-dismiss="modal"
										aria-label="Close"></button>
								</div>
								<div class="modal-body">
									<form:form class="form-horizontal form-label-left"
										method="POST" modelAttribute="payment_method"
										action="/admin/payment_method/create">
						...
						<div class="col-sm-12">
							<div class="card-box table-responsive">
								<table id="datatable-responsive"
									...
									<tbody>
										<c:forEach var="item" items="${list_payment_method}">
											<tr>
												<td>${item.ID }</td>
												<td>${item.NAME }</td>
												<td class="text-center"><i
													onclick="getpayment_methodById(${item.ID })"
								...
								<div class="modal fade" id="updateModel" tabindex="-1"
									...
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-body">
												<form:form class="form-horizontal form-label-left"
													method="POST" modelAttribute="payment_methodUpdate"
													...
													<div class="row text-left">
														...
														<div class="form-group">
															<label class="control-label">Tên phương thức thanh toán</label>
															<form:input id="payment_methodName" path="NAME" type="text"
	...
	<script>
		function getpayment_methodById(id) {
		    $.ajax({
		        type: "GET",
		        url: "/admin/ajax/getpayment_method/" + id,
		        success: function(payment_method) {
		            // Đổ dữ liệu vào form:form
		            $("#payment_methodId").val(payment_method.id);
		            $("#payment_methodName").val(payment_method.name); 
		...
		function modelDelete(id) {
		    $.ajax({
		        ...
		        success: function(payment_method) {
	                ...
	                $("#thongbao").text("Bạn có muốn xóa phương thức thanh toán:" + payment_method.name);
	                var modal = new bootstrap.Modal(document.getElementById('delete'));
	                modal.show();
	                
	                // Xử lý khi nhấn nút Delete
	                $("#deleteButton").on("click", function() {
	                    window.location.href = "/admin/payment_method/delete?id=" + payment_method.id;
	                });
		        },
		        error: function(xhr, status, error) {
		            console.log("Error: " + error);
	...
	<c:if test="${message eq 'Hoàn tất'}">
		<script>
			...
			setTimeout(function() {
				window.location.href = "/admin/payment_method";
			}, 1800);
